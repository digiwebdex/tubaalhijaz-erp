...
const NAVY = "var(--erp-text-strong)", GOLD = "var(--erp-accent)";
...
const inp = { backgroundColor: "var(--erp-canvas)", border: "1px solid rgba(11,30,63,0.15)", color: NAVY } as const;
...
export default function RateCards() {
  ...
  return (
    <div className="min-h-screen p-8" style={{ backgroundColor: "var(--erp-canvas)" }}>
      ...
      <div className="flex gap-1 mb-4">
        {(["transport", "visa", "additional"] as Kind[]).map((k) => (
          <button key={k} onClick={() => setKind(k)} className="px-4 py-2 text-xs font-semibold rounded-lg capitalize" style={{ backgroundColor: kind === k ? NAVY : "var(--erp-surface)", color: kind === k ? "var(--erp-surface)" : NAVY, border: "1px solid rgba(11,30,63,0.15)" }}>{k}</button>
        ))}
      </div>
        ...
        {rows === null && !error ? <div className="p-5"><LoadingSkeleton rows={4} /></div>
          ...
          : (
            <table className="w-full text-xs">
              <thead><tr style={{ backgroundColor: "var(--erp-surface-soft)" }}>{cols[kind].map((c) => <th key={c} className="px-4 py-2.5 text-left font-bold uppercase tracking-wider" style={{ color: "rgba(11,30,63,0.5)", fontSize: 9 }}>{c}</th>)}<th /></tr></thead>
              <tbody>
                {rows?.map((r) => (
                  <tr key={r.id} style={{ borderTop: "1px solid rgba(11,30,63,0.07)" }}>
                    ...
                    <td className="px-4 py-2.5 text-right whitespace-nowrap">
                      <button onClick={() => toggle(r)} className="text-[10px] font-semibold mr-3" style={{ color: r.active ? "var(--erp-warning)" : "var(--erp-success)" }}>{r.active ? "Deactivate" : "Activate"}</button>
                      <button onClick={() => del(r)} className="text-[10px] font-semibold" style={{ color: "var(--erp-destructive)" }}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
