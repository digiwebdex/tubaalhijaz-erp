...
export default function Footer() {
  ...
  return (
    <footer style={{ backgroundColor: "#141830" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            ...
            <div className="text-sm mb-6" style={{ color: "rgba(226,232,245,0.66)", fontFamily: ff, lineHeight: lh }}>
              {isBn
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {SERVICES[lang].map((s) => (
                <div key={s} className="text-sm cursor-pointer transition-colors" style={{ color: "rgba(226,232,245,0.66)", fontFamily: ff }}>
                  {s}
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              {CONTACT[lang].map((item, i) => (
                <div key={i} className="flex items-start gap-3">
                  ...
                  <span className="text-sm whitespace-pre-line" style={{ color: "rgba(226,232,245,0.66)", fontFamily: ff, lineHeight: lh }}>
                    {item.text}
                  </span>
                </div>
              ))}
            </div>
            <div className="mt-6 pt-6 border-t" style={{ borderColor: "rgba(226,232,245,0.11)" }}>
              <Link
                to="/login"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold"
                style={{ backgroundColor: GOLD, color: INK, fontFamily: ff }}
              >
        ...
        <div
          ...
          style={{ borderColor: "rgba(226,232,245,0.11)" }}
        >
          <div className="text-xs" style={{ color: "rgba(226,232,245,0.50)", fontFamily: ff }}>
            {isBn
          ...
          <div className="flex items-center gap-4">
            {[
              ...
              item.to
                ? <Link key={item.en} to={item.to} className="text-xs" style={{ color: "rgba(226,232,245,0.38)", fontFamily: ff }}>{isBn ? item.bn : item.en}</Link>
                : <span key={item.en} className="text-xs cursor-pointer" style={{ color: "rgba(226,232,245,0.50)", fontFamily: ff }}>{isBn ? item.bn : item.en}</span>
            ))}
          </div>
        </div>
      </div>
